  ...
  type:"POST",
    ...
    success: function (response){
      if(response.status==200){
         ...
         if(isAccepted){
             window.open("https://github.com/login/oauth/authorize?client_id=723ca7c1206fcfbff6cb&redirect_uri=http://47.93.60.108:80/callback&scop=user");
    ...
    complete:function (){
        $("#comment-main").load("http://47.93.60.108:80/question/"+id+" #comment-origin");
    },
    dataType:"json"
    })
}
...
function subCommentShow(e){
    ...
    if(collapse){
        ...
        if($("#subComment-"+id).children().length===1){
            ...
            $.ajax({
                type:"POST",
                url:"http://47.93.60.108:80/subComment/"+id,
                dataType: "json",
                success: function (response){
...
function subComment(e){
    ...
    $.ajax({
        type:"POST",
        contentType: "application/json",
        url:"http://47.93.60.108:80/comment",
        data: JSON.stringify(
            {
            "type":1,
            "parentId":id,
